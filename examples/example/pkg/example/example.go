...
func Init() error {
	...
	if err != nil {
		return fmt.Errorf("CreateModule example: %s", err)
	}

	if err := exampleClass.Create(); err != nil {
		return fmt.Errorf("exampleClass.Create: %s", err)
	}

	if err = m.AddObjectRef("ExampleClass", &exampleClass); err != nil {
		return fmt.Errorf("Add ExampleClass: %s", err)
	}

	if err := dictClass.Create(); err != nil {
		return fmt.Errorf("dictClass.Create: %s", err)
	}

	if err = m.AddObjectRef("MyDict", &dictClass); err != nil {
		return fmt.Errorf("Add MyDict: %s", err)
	}

	if err := subClass.Create(); err != nil {
		return fmt.Errorf("subClass.Create: %s", err)
	}

	if err = m.AddObjectRef("SubClass", &subClass); err != nil {
		return fmt.Errorf("Add SubClass: %s", err)
	}

	if err := m.Register(); err != nil {
		return fmt.Errorf("Register example: %s", err)
	}

	s, err := py.CreateModule(&subDef)
	if err != nil {
		return fmt.Errorf("CreateModule sub: %s", err)
	}

	if err := s.Register(); err != nil {
		return fmt.Errorf("Register sub: %s", err)
	}

	if _, err := py.InitGoModule(); err != nil {
		return fmt.Errorf("InitGoModule: %s", err)
	}
