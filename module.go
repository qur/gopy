...
// FilenameObject returns the name of the file form which m was loaded using m's
// __file__ attribute. If this is not defined, or if it is not a string, returns
// a SystemError.
func (m *Module) FilenameObject() (*Unicode, error) {
	fn, err := obj2ObjErr(C.PyModule_GetFilenameObject(c(m)))
	if err != nil {
		return nil, err
	}

	if fnu, ok := fn.(*Unicode); ok {
		return fnu, nil
	}

	return nil, TypeError.Err("expected str, got %s", fn.Type())
}
