...
func process_imports(name string, imports []imp) {
	fmt.Printf("#include <python2.7/Python.h>\n")
	fmt.Printf("\n")
	fmt.Printf("extern void _init_go(void *funcs[]);\n")
	for _, i := range imports {
		fmt.Printf("extern void __%s_init(void) __asm__ (\"%s\");\n", i.n, i.f)
	}
	fmt.Printf("\n")
	fmt.Printf("static void *funcs[] = {\n")
	for _, i := range imports {
	...
	fmt.Printf("};\n")
	fmt.Printf("\n")
	fmt.Printf("PyMODINIT_FUNC init%s(void)\n", name)
	fmt.Printf("{\n")
	fmt.Printf("    _init_go(funcs);\n")
	fmt.Printf("}\n")
}
